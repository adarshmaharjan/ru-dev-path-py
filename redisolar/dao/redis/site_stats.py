...
from redisolar.models import MeterReading, SiteStats
...
class SiteStatsDaoRedis(SiteStatsDaoBase, RedisDaoBase):
    ...
    def find_by_id(
        ...
        return cast(SiteStats, SiteStatsSchema().load(cast(Mapping[str, Any], fields)))
    ...
    def _update_optimized(
        ...
        execute = False
        if pipeline is None:
            pipeline = cast(redis.client.Pipeline, self.redis.pipeline())
            execute = True
